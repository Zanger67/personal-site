...
// components or pages.
//
// This file has the SAME SHAPE in the template (personal-site-template) repo;
...
//   • A disabled home section / footer link simply isn't rendered. Homepage
//     "preview" sections also hide when their target route is disabled.
//
...
export const footerLinks = [
  { label: 'GitHub',   href: 'https://github.com/Zanger67', external: true,  enabled: true  },
  { label: 'LinkedIn', href: 'https://linkedin.com/in/zang', external: true,  enabled: true  },
  { label: 'Email',    href: '/contact',                     external: false, enabled: true  },
  { label: 'RSS',      href: '/rss.xml',                     external: false, enabled: false }, // blog is WIP
...
// data to show (the page keeps its existing `length > 0` guards). A "preview"
// section that funnels to a route also hides when that route is disabled.
// Switching a flag off here force-hides the block regardless of data.
...
export const authorInstitutions = {
  ...
  // The registry keeps both either way — this only picks which one prints. An
  // institution that should stay spelled out under 'short' just writes its full
  // name as its `short` (see gt-ai-safety-initiative in institutions.json).
  legendNames: 'short',
} as const;
